...
fn governance_to_storage_end_to_end_loop() {
    let ledger_path =
        std::env::temp_dir().join(format!("sovereign_system_e2e_{}.jsonl", Uuid::new_v4()));

    let governance =
        GovernanceEngine::open(&ledger_path).expect("failed to initialize governance engine");
    ...
    let raw = std::fs::read_to_string(&ledger_path).expect("failed to read ledger file");
