...
fn test_persistence_lifecycle_loop() {
    let test_file_path = std::env::temp_dir().join(format!(
        "sovereign_test_lifecycle_{}.jsonl",
        uuid::Uuid::new_v4()
    ));

    let engine = StorageEngine::new(&test_file_path).expect("Failed to initialize storage engine");

    let event_1 = ActiveEvent::new(
        "NODE_init",
        json!({ "node_id": "test-node-01", "status": "active" }),
    );
    let event_2 = ActiveEvent::new(
        "METRICS_snapshot",
        json!({ "metrics": { "cpu_utilization": 12.5 } }),
    );

    engine
        .append_event(&event_1)
        .expect("Failed to append event 1");
    engine
        .append_event(&event_2)
        .expect("Failed to append event 2");

    let recovered_events = engine
        .load_events()
        .expect("Failed to load events from disk");
...
fn test_empty_log_returns_empty_vector() {
    let test_file_path = std::env::temp_dir().join(format!(
        "sovereign_test_empty_{}.jsonl",
        uuid::Uuid::new_v4()
    ));

    let engine =
        StorageEngine::new(&test_file_path).expect("Failed to initialize empty storage engine");
    let recovered_events = engine
        .load_events()
        .expect("Loading an empty file must succeed");
...
fn test_corrupted_log_line_surfaces_serialization_error() {
    ...
    engine
        .append_event(&event)
        .expect("Failed to write valid event");
    ...
    match engine.load_events() {
        ...
        other => panic!(
            "Expected ActiveMemoryError::Serialization, got: {:?}",
            other
        ),
    }
...
fn test_deep_nested_directory_auto_creation() {
    let root = std::env::temp_dir().join(format!("gusa_nested_layer_a_{}", uuid::Uuid::new_v4()));
    ...
    engine
        ...
        .expect("Failed to write nested event");
