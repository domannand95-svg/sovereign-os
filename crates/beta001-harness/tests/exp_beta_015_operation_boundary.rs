...
fn test_exp_015_valid_operation_match_request() {
    ...
    let valid_path = Path::new(manifest_dir)
        .join("tests/fixtures/exp_beta_015/valid/valid_operation_match_001.json");

    let content =
        fs::read_to_string(&valid_path).expect("Failed to read valid operation match fixture");
    let instance: Value = serde_json::from_str(&content).expect("Failed to parse JSON");
    assert!(
        validator.is_valid(&instance),
        "Valid operation match request rejected"
    );
}
...
fn test_exp_015_invalid_operation_match_request() {
    ...
    let invalid_path = Path::new(manifest_dir)
        .join("tests/fixtures/exp_beta_015/invalid/invalid_scope_escalation.json");

    let content =
        fs::read_to_string(&invalid_path).expect("Failed to read invalid operation match fixture");
    let instance: Value = serde_json::from_str(&content).expect("Failed to parse JSON");
    assert!(
        !validator.is_valid(&instance),
        "Adversarial operation match request incorrectly accepted"
    );
}
