...
impl ReceiptEpistemicValidator {
    pub fn validate(receipt: &Value, expected_candidate_y: &str) -> Result<(), String> {
        ...
        let prov = receipt
            ...
            .ok_or("RECEIPT_INVALID: Missing provenance_chain")?;
        prov.get("candidate_digest")
            .ok_or("RECEIPT_INVALID: Missing candidate_digest")?;

        let lease_ref = prov
            .get("lease_reference")
            .ok_or("RECEIPT_INVALID: Missing lease_reference")?;
        ...
        receipt
            ...
            .ok_or("RECEIPT_INVALID: Missing authorization_consumption")?;

        let exec_obs = receipt
            .get("execution_observation")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let term_disp = receipt
            .get("terminal_disposition")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let post_obs = receipt
            .get("independent_post_observation")
            .ok_or("RECEIPT_INVALID: Missing independent_post_observation")?;
        let obs_state = post_obs
...
fn test_tc_pub_receipt_001_valid_verified_success_accepted() {
    assert!(ReceiptEpistemicValidator::validate(
        &valid_receipt_base(),
...
fn test_tc_pub_receipt_002_missing_provenance_rejected() {
    ...
    let res =
        ReceiptEpistemicValidator::validate(&receipt, "cccccccccccccccccccccccccccccccccccccccc");
    assert_eq!(
        res.unwrap_err(),
        "RECEIPT_INVALID: Missing provenance_chain"
    );
}
...
fn test_tc_pub_receipt_003_success_lie_rejected() {
    ...
    receipt["independent_post_observation"]["observed_oid"] =
    ...
    assert_eq!(
        res.unwrap_err(),
        "EPISTEMIC_INVALID: VERIFIED_SUCCESS requires independent observation of candidate OID"
    );
}
...
fn test_tc_pub_receipt_004_verified_success_with_unknown_observation_rejected() {
    ...
    receipt["independent_post_observation"]
        .as_object_mut()
        .unwrap()
        .remove("observed_oid");
    let res =
...
fn test_tc_pub_receipt_007_fake_lease_none_rejected() {
    ...
    assert_eq!(
        res.unwrap_err(),
        "RECEIPT_INVALID: lease_reference cannot be a string (e.g., NONE)"
    );
}
...
fn test_tc_pub_receipt_008_missing_consumption_state_rejected() {
    ...
    receipt
        .as_object_mut()
        .unwrap()
        .remove("authorization_consumption");
    let res =
        ReceiptEpistemicValidator::validate(&receipt, "cccccccccccccccccccccccccccccccccccccccc");
    assert_eq!(
        res.unwrap_err(),
        "RECEIPT_INVALID: Missing authorization_consumption"
    );
}
