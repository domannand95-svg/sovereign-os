...
fn test_exp_014_valid_execution_request() {
    ...
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let valid_path = Path::new(manifest_dir).join("tests/fixtures/exp_beta_014/valid/valid_execution_001.json");
    ...
    assert!(validator.is_valid(&instance), "Valid execution request rejected");
}
...
fn test_exp_014_invalid_execution_request() {
    let validator = load_schema("CAPABILITY_EXECUTION_REQUEST-v1.schema.json");
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let invalid_path = Path::new(manifest_dir).join("tests/fixtures/exp_beta_014/invalid/invalid_scope_and_credential.json");

    let content = fs::read_to_string(&invalid_path).expect("Failed to read invalid execution fixture");
    let instance: Value = serde_json::from_str(&content).expect("Failed to parse JSON");
    assert!(!validator.is_valid(&instance), "Adversarial execution request incorrectly accepted");
}
