...
impl PullRequestAuthorizationValidator {
    pub fn validate(value: &serde_json::Value) -> AuthorizationValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str()) != Some("REPOSITORY_PULL_REQUEST_AUTHORIZATION-v1") {
            return AuthorizationValidationResult::Invalid("Invalid or missing schema_version".into());
        }

        // Validate authorization_id pattern
        if let Some(id) = value.get("authorization_id").and_then(|v| v.as_str()) {
            if !id.starts_with("pr_auth_") {
                return AuthorizationValidationResult::Invalid("Invalid authorization_id format".into());
            }
        } else {
        ...
        if let Some(scope) = value.get("authorized_scope") {
            if scope.get("operation").and_then(|v| v.as_str()) != Some("repository.remote.create_pull_request") {
                return AuthorizationValidationResult::Invalid("Invalid or unauthorized operation scope".into());
            }
            if scope.get("review_permitted").and_then(|v| v.as_bool()) != Some(false) {
                return AuthorizationValidationResult::Invalid("Violation: review_permitted must be false".into());
            }
            if scope.get("approval_permitted").and_then(|v| v.as_bool()) != Some(false) {
                return AuthorizationValidationResult::Invalid("Violation: approval_permitted must be false".into());
            }
            if scope.get("merge_permitted").and_then(|v| v.as_bool()) != Some(false) {
                return AuthorizationValidationResult::Invalid("Violation: merge_permitted must be false".into());
            }
        } else {
        ...
        if let Some(bounds) = value.get("temporal_bounds") {
            ...
            if let (Some(issued), Some(expires)) = (issued_str, expires_str) {
                ...
                if let (Ok(i), Ok(e)) = (issued_dt, expires_dt) {
                    if e <= i {
                        return AuthorizationValidationResult::Invalid("Temporal bounds violation: expires_at must be after issued_at".into());
                    }
                    if e < Utc::now() {
                        return AuthorizationValidationResult::Invalid("Authorization expired".into());
                    }
                } else {
                    return AuthorizationValidationResult::Invalid("Invalid RFC3339 date format in temporal bounds".into());
                }
            } else {
                return AuthorizationValidationResult::Invalid("Missing issued_at or expires_at".into());
            }
        } else {
        ...
        let allowed_keys = [
            ...
            "target_binding", "temporal_bounds", "authorized_scope", "consumption_policy"
        ];
        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return AuthorizationValidationResult::Invalid(format!("Injected unauthorized authorization field detected: {}", key));
                }
            }
        }
...
mod pr_authorization_schema_tests {
    ...
    fn tc_pr_auth_001_valid_authorization_accepted() {
        ...
        assert_eq!(PullRequestAuthorizationValidator::validate(&auth), AuthorizationValidationResult::Valid);
    }
    ...
    fn tc_pr_auth_002_missing_candidate_binding_rejected() {
        ...
        assert!(matches!(PullRequestAuthorizationValidator::validate(&auth), AuthorizationValidationResult::Invalid(_)));
    }
