...
impl ReviewObservationCandidateValidator {
    pub fn validate(value: &serde_json::Value) -> ReviewCandidateValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str())
            != Some("REPOSITORY_REVIEW_OBSERVATION_CANDIDATE-v1")
        {
            return ReviewCandidateValidationResult::Invalid(
        ...
        if let Some(id) = value.get("review_observation_id").and_then(|v| v.as_str()) {
            if !id.starts_with("rev_obs_") {
                return ReviewCandidateValidationResult::Invalid(
                    "Invalid review_observation_id format".into(),
                );
            }
        } else {
            return ReviewCandidateValidationResult::Invalid(
                "Missing review_observation_id".into(),
            );
        }
        ...
        if let Some(state) = value.get("review_state").and_then(|v| v.as_str()) {
            ...
            if !valid_states.contains(&state) {
                return ReviewCandidateValidationResult::Invalid(
                    "Invalid review_state value".into(),
                );
            }
        } else {
        ...
        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return ReviewCandidateValidationResult::Invalid(format!(
...
mod review_candidate_schema_tests {
    ...
    fn tc_rev_cand_001_valid_observation_accepted() {
        ...
        assert_eq!(
            ReviewObservationCandidateValidator::validate(&obs),
            ReviewCandidateValidationResult::Valid
        );
    }
    ...
    fn tc_rev_cand_002_missing_pr_binding_rejected() {
        ...
        assert!(matches!(
            ReviewObservationCandidateValidator::validate(&obs),
            ReviewCandidateValidationResult::Invalid(_)
        ));
    }
    ...
    fn tc_rev_cand_004_injected_merge_authority_rejected() {
        ...
        obs.as_object_mut()
            .unwrap()
            .insert("grants_merge_authority".to_string(), json!(true));
        assert!(matches!(
            ReviewObservationCandidateValidator::validate(&obs),
            ReviewCandidateValidationResult::Invalid(_)
        ));
