...
fn test_exp_016_valid_state_query() {
    ...
    let valid_path = Path::new(manifest_dir)
        .join("tests/fixtures/exp_beta_016/valid/valid_state_query_001.json");

    let content =
        fs::read_to_string(&valid_path).expect("Failed to read valid state query fixture");
...
fn test_exp_016_invalid_state_query() {
    ...
    let invalid_path = Path::new(manifest_dir)
        .join("tests/fixtures/exp_beta_016/invalid/invalid_renewal_request.json");

    let content =
        fs::read_to_string(&invalid_path).expect("Failed to read invalid state query fixture");
    let instance: Value = serde_json::from_str(&content).expect("Failed to parse JSON");
    assert!(
        !validator.is_valid(&instance),
        "Adversarial state query (renewal attempt) incorrectly accepted"
    );
}
