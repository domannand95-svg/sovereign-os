...
impl MergeCandidateValidator {
    pub fn validate(value: &serde_json::Value) -> MergeCandidateValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str()) != Some("REPOSITORY_MERGE_CANDIDATE-v1") {
            return MergeCandidateValidationResult::Invalid("Invalid or missing schema_version".into());
        }
        ...
        if let Some(id) = value.get("merge_candidate_id").and_then(|v| v.as_str()) {
            if !id.starts_with("mrg_cand_") {
                return MergeCandidateValidationResult::Invalid("Invalid merge_candidate_id format".into());
            }
        } else {
        ...
        if let Some(oid) = value.get("expected_prestate_oid").and_then(|v| v.as_str()) {
            if oid.len() != 40 {
                return MergeCandidateValidationResult::Invalid("Invalid expected_prestate_oid length".into());
            }
        } else {
        ...
        if let Some(strat) = value.get("declared_strategy").and_then(|v| v.as_str()) {
            ...
            if !valid_strategies.contains(&strat) {
                return MergeCandidateValidationResult::Invalid("Invalid declared_strategy value".into());
            }
        } else {
        ...
        let allowed_keys = [
            "schema_version", "merge_candidate_id", "pull_request_binding",
            "target_repository_identity", "source_ref", "target_ref",
            "expected_prestate_oid", "declared_strategy"
        ];
        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return MergeCandidateValidationResult::Invalid(format!("Injected unauthorized authority field detected: {}", key));
                }
            }
        }
...
mod merge_candidate_schema_tests {
    ...
    fn tc_mrg_cand_001_valid_candidate_accepted() {
        ...
        assert_eq!(MergeCandidateValidator::validate(&cand), MergeCandidateValidationResult::Valid);
    }
    ...
    fn tc_mrg_cand_002_missing_pr_binding_rejected() {
        ...
        assert!(matches!(MergeCandidateValidator::validate(&cand), MergeCandidateValidationResult::Invalid(_)));
    }
    ...
    fn tc_mrg_cand_004_injected_execution_trigger_rejected() {
        let mut cand = get_valid_merge_candidate();
        cand.as_object_mut().unwrap().insert("trigger_deployment".to_string(), json!(true));
        assert!(matches!(MergeCandidateValidator::validate(&cand), MergeCandidateValidationResult::Invalid(_)));
    }
    ...
    fn tc_mrg_cand_006_missing_prestate_oid_rejected() {
        let mut cand = get_valid_merge_candidate();
        cand.as_object_mut().unwrap().remove("expected_prestate_oid");
        assert!(matches!(MergeCandidateValidator::validate(&cand), MergeCandidateValidationResult::Invalid(_)));
    }
