...
fn test_exp_017_valid_audit_event() {
    ...
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let valid_path = Path::new(manifest_dir).join("tests/fixtures/exp_beta_017/valid/valid_execution_audit_001.json");

    let content = fs::read_to_string(&valid_path).expect("Failed to read valid audit event fixture");
...
fn test_exp_017_invalid_audit_event() {
    let validator = load_schema("CAPABILITY_EXECUTION_AUDIT_EVENT-v1.schema.json");
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let invalid_path = Path::new(manifest_dir).join("tests/fixtures/exp_beta_017/invalid/invalid_missing_lineage_and_token.json");

    let content = fs::read_to_string(&invalid_path).expect("Failed to read invalid audit event fixture");
    let instance: Value = serde_json::from_str(&content).expect("Failed to parse JSON");
    assert!(!validator.is_valid(&instance), "Adversarial audit event incorrectly accepted");
}
