...
impl WorkflowCandidateValidator {
    pub fn validate(value: &serde_json::Value) -> WorkflowCandidateValidationResult {
        ...
        match value.get("participating_domains").and_then(|v| v.as_array()) {
            Some(domains) if !domains.is_empty() => (),
            _ => return WorkflowCandidateValidationResult::Invalid("Missing or empty participating_domains".into()),
        }
        ...
        if let Some(refs) = value.get("policy_references").and_then(|v| v.as_array()) {
            for r in refs {
                if r.get("policy_digest").and_then(|v| v.as_str()).map_or(true, |d| !d.starts_with("sha256:")) {
                    return WorkflowCandidateValidationResult::Invalid("Floating or unpinned policy digest reference".into());
                }
            }
        }
        ...
        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return WorkflowCandidateValidationResult::Invalid(format!("Authority injection or forbidden field detected: {}", key));
                }
            }
        }
        ...
        if let Some(intent) = value.get("coordination_intent").and_then(|v| v.as_object()) {
            ...
            {
                return WorkflowCandidateValidationResult::Invalid("Forbidden operational execution directive in intent".into());
            }
        }
...
mod workflow_candidate_schema_tests {
    ...
    fn tc_workflow_cand_002_missing_domains_rejected() {
        ...
        assert!(matches!(WorkflowCandidateValidator::validate(&empty_domains), WorkflowCandidateValidationResult::Invalid(_)));
    }
    ...
    fn tc_workflow_cand_004_authority_injection_rejected() {
        let mut cand = get_valid_workflow_candidate();
        cand.as_object_mut().unwrap().insert("grant_capability".to_string(), json!("root"));
        assert!(matches!(WorkflowCandidateValidator::validate(&cand), WorkflowCandidateValidationResult::Invalid(_)));
    }
