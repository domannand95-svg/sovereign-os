...
impl AgentCommunicationValidator {
    pub fn validate(value: &serde_json::Value) -> AgentCommunicationValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str()) != Some("REPOSITORY_AGENT_COMMUNICATION_EVIDENCE-v1") {
            return AgentCommunicationValidationResult::Invalid("Invalid or missing schema_version".into());
        }

        // Validate message_id pattern
        if let Some(id) = value.get("message_id").and_then(|v| v.as_str()) {
            if !id.starts_with("msg_") {
                return AgentCommunicationValidationResult::Invalid("Invalid message_id format".into());
            }
        } else {
        ...
        if let Some(sender) = value.get("sender_agent_identity").and_then(|v| v.as_str()) {
            if !sender.starts_with("agent_") {
                return AgentCommunicationValidationResult::Invalid("Invalid sender identity format".into());
            }
        } else {
            return AgentCommunicationValidationResult::Invalid("Missing sender_agent_identity".into());
        }
        ...
        let allowed_keys = [
            "schema_version", "message_id", "sender_agent_identity",
            "communication_context_reference", "message_type",
            "epistemic_classification", "provenance"
        ];

        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return AgentCommunicationValidationResult::Invalid(format!("Authority injection or forbidden instruction field detected: {}", key));
                }
            }
        }
        ...
        if let Some(provenance) = value.get("provenance") {
            if let Some(digest) = provenance.get("message_digest").and_then(|v| v.as_str()) {
                if !digest.starts_with("sha256:") || digest.len() != 71 {
                    return AgentCommunicationValidationResult::Invalid("Invalid or missing message digest format".into());
                }
            } else {
                return AgentCommunicationValidationResult::Invalid("Missing message_digest in provenance".into());
            }
        } else {
...
mod agent_communication_evidence_tests {
    ...
    fn tc_agent_comm_001_valid_communication_evidence_accepted() {
        ...
        assert_eq!(AgentCommunicationValidator::validate(&msg), AgentCommunicationValidationResult::Valid);
    }
    ...
    fn tc_agent_comm_002_reject_unknown_sender_identity() {
        ...
        assert!(matches!(AgentCommunicationValidator::validate(&msg), AgentCommunicationValidationResult::Invalid(_)));
    }

    #[test]
    fn tc_agent_comm_003_reject_embedded_authority_instructions() {
        let mut msg = get_valid_communication_message();
        msg.as_object_mut().unwrap().insert("approve_merge".to_string(), json!(true));
        assert!(matches!(AgentCommunicationValidator::validate(&msg), AgentCommunicationValidationResult::Invalid(_)));
    }
    ...
    fn tc_agent_comm_004_reject_missing_provenance_digest() {
        let mut msg = get_valid_communication_message();
        msg["provenance"].as_object_mut().unwrap().remove("message_digest");
        assert!(matches!(AgentCommunicationValidator::validate(&msg), AgentCommunicationValidationResult::Invalid(_)));
    }
    ...
    fn tc_agent_comm_005_reject_impersonated_sender_identity() {
        ...
        assert!(matches!(AgentCommunicationValidator::validate(&invalid_sender), AgentCommunicationValidationResult::Invalid(_)));
    }

    #[test]
    fn tc_agent_comm_006_reject_message_to_capability_escalation() {
        let mut msg = get_valid_communication_message();
        msg.as_object_mut().unwrap().insert("grant_capability".to_string(), json!("admin"));
        assert!(matches!(AgentCommunicationValidator::validate(&msg), AgentCommunicationValidationResult::Invalid(_)));
    }
    ...
    fn tc_agent_comm_007_reject_trust_inheritance_from_history() {
        let mut msg = get_valid_communication_message();
        msg.as_object_mut().unwrap().insert("inherit_historical_trust".to_string(), json!(true));
        assert!(matches!(AgentCommunicationValidator::validate(&msg), AgentCommunicationValidationResult::Invalid(_)));
    }
}
