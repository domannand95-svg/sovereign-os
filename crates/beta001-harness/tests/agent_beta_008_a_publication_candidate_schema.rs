...
impl CandidateSemanticValidator {
    pub fn validate(candidate: &Value) -> Result<(), String> {
        ...
        let cand_oid = candidate["candidate_commit_oid"]
            .as_str()
            .ok_or("Missing candidate_commit_oid")?;
        ...
        let state_type = expected_state["state_type"]
            .as_str()
            .ok_or("Missing state_type")?;
...
fn test_tc_pub_cand_005_expected_remote_state_mismatch_rejected() {
    ...
    assert_eq!(
        res.unwrap_err(),
        "expected_remote_state.oid != proposed_transition.from_oid"
    );
}
...
fn test_tc_pub_cand_006_candidate_oid_mismatch_rejected() {
    ...
    assert_eq!(
        res.unwrap_err(),
        "candidate_commit_oid != proposed_transition.to_oid"
    );
}
...
fn test_tc_pub_cand_008_absent_state_with_oid_rejected() {
    let mut cand = valid_candidate_base();
    cand["expected_remote_state"] =
        json!({ "state_type": "absent", "oid": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" });
