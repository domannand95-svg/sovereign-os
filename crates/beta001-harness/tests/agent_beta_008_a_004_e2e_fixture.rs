...
impl<'a> SovereignPublicationOrchestrator<'a> {
    pub fn execute_governed_publication(
        ...
        let exec_obs = self
            .adapter
            .dispatch_exact(&transport_req, self.credentials);
...
mod e2e_tests {
    ...
    impl NetworkTransport for E2EFixture {
        fn execute_push(
            ...
            if !captured {
                return Err(NetworkError::CredentialFailure);
            }
    ...
    impl IndependentRemoteVerifier for E2EFixture {
        fn observe_remote_state(&self, _dest_ref: &str) -> IndependentPostObservation {
            ...
            let oid = if count == 0 {
                &self.verifier_pre_oid
            } else {
                &self.verifier_post_oid
            };
    ...
    fn default_run(
        fixture: &E2EFixture,
        cand_override: Option<RepositoryPublicationCandidate>,
        auth_override: Option<RepositoryPublicationAuthorization>,
        lease_override: Option<RepositoryCredentialLease>,
    ) -> RepositoryPublicationReceipt {
        ...
        let adapter = SovereignGitAdapter {
            network_client: fixture,
        };
    ...
    fn suite_1_happy_path() {
        let fixture = E2EFixture {
            network_result: Ok(()),
            verifier_pre_oid: "X".into(),
            verifier_post_oid: "Y".into(),
    ...
    fn suite_2_cas_race_prevents_dispatch() {
        ...
        let fixture = E2EFixture {
            network_result: Ok(()),
            verifier_pre_oid: "Z".into(),
            verifier_post_oid: "Z".into(),
            secret: Some("token".into()),
            calls: Cell::new(0),
        };
        let receipt = default_run(&fixture, None, None, None);
    ...
    fn suite_4_false_negative_resolved_to_success() {
        ...
        let fixture = E2EFixture {
            network_result: Err(NetworkError::ProtocolViolation),
            verifier_pre_oid: "X".into(),
            verifier_post_oid: "Y".into(),
            secret: Some("token".into()),
            calls: Cell::new(0),
    ...
    fn suite_5_expired_lease_denied() {
        ...
        assert_eq!(
            receipt.terminal_disposition,
            TerminalDisposition::CredentialUnavailable
        );
        assert_eq!(
            receipt.execution_observation,
            ExecutionObservation::NotDispatched
    ...
    fn suite_6_escalation_force_update_denied() {
        ...
        let bad_auth = RepositoryPublicationAuthorization {
            authorization_id: "auth_01".into(),
            authorized_candidate_id: "cand_01".into(),
            operation: "repository.remote.force_update".into(),
        };
