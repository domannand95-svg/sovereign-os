...
impl ReviewAuthorizationValidator {
    pub fn validate(value: &serde_json::Value) -> ReviewAuthorizationValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str()) != Some("REPOSITORY_REVIEW_AUTHORIZATION-v1") {
            return ReviewAuthorizationValidationResult::Invalid("Invalid or missing schema_version".into());
        }
        ...
        if let Some(id) = value.get("authorization_id").and_then(|v| v.as_str()) {
            if !id.starts_with("review_auth_") {
                return ReviewAuthorizationValidationResult::Invalid("Invalid authorization_id format".into());
            }
        } else {
        ...
        if value.get("candidate_binding").is_none() {
            return ReviewAuthorizationValidationResult::Invalid("Missing candidate_binding".into());
        }
        ...
        if let Some(scope) = value.get("authorized_scope") {
            ...
            if op != Some("repository.remote.review.observe") && op != Some("repository.remote.review.submit_comment") {
                return ReviewAuthorizationValidationResult::Invalid("Invalid or unauthorized operation scope".into());
            }
            if scope.get("approval_permitted").and_then(|v| v.as_bool()) != Some(false) {
                return ReviewAuthorizationValidationResult::Invalid("Violation: approval_permitted must be false".into());
            }
            if scope.get("merge_permitted").and_then(|v| v.as_bool()) != Some(false) {
                return ReviewAuthorizationValidationResult::Invalid("Violation: merge_permitted must be false".into());
            }
        } else {
        ...
        if let Some(bounds) = value.get("temporal_bounds") {
            ...
            if let (Some(issued), Some(expires)) = (issued_str, expires_str) {
                ...
                if let (Ok(i), Ok(e)) = (issued_dt, expires_dt) {
                    if e <= i {
                        return ReviewAuthorizationValidationResult::Invalid("Temporal bounds violation: expires_at must be after issued_at".into());
                    }
                    if e < Utc::now() {
                        return ReviewAuthorizationValidationResult::Invalid("Authorization expired".into());
                    }
                } else {
                    return ReviewAuthorizationValidationResult::Invalid("Invalid RFC3339 date format in temporal bounds".into());
                }
            } else {
                return ReviewAuthorizationValidationResult::Invalid("Missing issued_at or expires_at".into());
            }
        } else {
        ...
        if value.get("consumption_policy").and_then(|p| p.get("single_use_only")).and_then(|v| v.as_bool()) != Some(true) {
            return ReviewAuthorizationValidationResult::Invalid("single_use_only must be true".into());
        }
        ...
        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return ReviewAuthorizationValidationResult::Invalid(format!("Injected unauthorized review authorization field detected: {}", key));
                }
            }
        }
...
mod review_authorization_schema_tests {
    ...
    fn tc_rev_auth_001_valid_authorization_accepted() {
        ...
        assert_eq!(ReviewAuthorizationValidator::validate(&auth), ReviewAuthorizationValidationResult::Valid);
    }
    ...
    fn tc_rev_auth_002_missing_candidate_binding_rejected() {
        ...
        assert!(matches!(ReviewAuthorizationValidator::validate(&auth), ReviewAuthorizationValidationResult::Invalid(_)));
    }
    ...
    fn tc_rev_auth_008_reviewer_identity_impersonation_rejected() {
        let mut auth = get_valid_review_authorization();
        auth.as_object_mut().unwrap().insert("impersonate_principal".to_string(), json!("admin_user"));
        assert!(matches!(ReviewAuthorizationValidator::validate(&auth), ReviewAuthorizationValidationResult::Invalid(_)));
    }
