...
impl ExternalEvidenceValidator {
    pub fn validate(value: &serde_json::Value) -> ExternalEvidenceValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str()) != Some("REPOSITORY_EXTERNAL_EVIDENCE-v1") {
            return ExternalEvidenceValidationResult::Invalid("Invalid or missing schema_version".into());
        }
        ...
        if let Some(id) = value.get("external_evidence_id").and_then(|v| v.as_str()) {
            if !id.starts_with("ext_evid_") {
                return ExternalEvidenceValidationResult::Invalid("Invalid external_evidence_id format".into());
            }
        } else {
            return ExternalEvidenceValidationResult::Invalid("Missing external_evidence_id".into());
        }
        ...
        if let Some(src) = value.get("source_identity") {
            if let Some(provider) = src.get("provider_id").and_then(|v| v.as_str()) {
                if provider.is_empty() {
                    return ExternalEvidenceValidationResult::Invalid("Anonymous evidence: provider_id cannot be empty".into());
                }
            } else {
                return ExternalEvidenceValidationResult::Invalid("Missing provider_id in source_identity".into());
            }
        } else {
            return ExternalEvidenceValidationResult::Invalid("Missing source_identity provenance binding".into());
        }
        ...
        let allowed_keys = [
            "schema_version", "external_evidence_id", "source_identity",
            "attestation_content", "epistemic_status", "temporal_bounds"
        ];

        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return ExternalEvidenceValidationResult::Invalid(format!("Authority injection or forbidden privilege field detected: {}", key));
                }
            }
        }
        ...
        if let Some(attestation) = value.get("attestation_content") {
            if let Some(digest) = attestation.get("content_digest").and_then(|v| v.as_str()) {
                if !digest.starts_with("sha256:") || digest.len() != 71 {
                    return ExternalEvidenceValidationResult::Invalid("Invalid content digest format".into());
                }
            } else {
                return ExternalEvidenceValidationResult::Invalid("Missing content_digest in attestation".into());
            }
        } else {
...
mod external_evidence_schema_tests {
    ...
    fn tc_ext_evid_001_valid_external_evidence_accepted() {
        ...
        assert_eq!(ExternalEvidenceValidator::validate(&evid), ExternalEvidenceValidationResult::Valid);
    }
    ...
    fn tc_ext_evid_002_reject_missing_source_provenance() {
        ...
        assert!(matches!(ExternalEvidenceValidator::validate(&evid), ExternalEvidenceValidationResult::Invalid(_)));
    }
    ...
    fn tc_ext_evid_004_reject_embedded_authority_grants() {
        let mut evid = get_valid_external_evidence();
        evid.as_object_mut().unwrap().insert("grant_capability".to_string(), json!("repository.deploy"));
        assert!(matches!(ExternalEvidenceValidator::validate(&evid), ExternalEvidenceValidationResult::Invalid(_)));
    }
    ...
    fn tc_ext_evid_005_reject_execution_permission_injection() {
        let mut evid = get_valid_external_evidence();
        evid.as_object_mut().unwrap().insert("approve_deployment".to_string(), json!(true));
        assert!(matches!(ExternalEvidenceValidator::validate(&evid), ExternalEvidenceValidationResult::Invalid(_)));
    }
