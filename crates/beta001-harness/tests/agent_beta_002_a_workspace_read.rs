...
impl WorkspaceReadAdapter {
    ...
    pub fn execute_read(
        ...
        if !is_grant_active || is_revoked {
            return Err(ReadError::AuthorityDenied(
                "Grant is inactive, expired, or revoked".to_string(),
            ));
        }
        ...
        if !normalized.starts_with(&normalized_base) {
            return Err(ReadError::ScopeViolation(
                "Target path escapes granted sandbox scope".to_string(),
            ));
        }
...
fn test_agent_002_a01_read_exact_authorized_file_succeeds() {
    ...
    assert!(
        res.is_ok(),
        "Authorized read within scope rejected: {:?}",
        res
    );
}
...
fn test_agent_002_a02_read_outside_granted_root_denied() {
    ...
    let res = adapter.execute_read(
        "sandbox/session-001/input",
        "../../../etc/passwd",
        true,
        false,
    );
...
fn test_agent_002_data_authority_separation() {
    ...
    assert!(
        !malicious_file_content.contains("HOST_AUTHORITY_MUTATION"),
        "Data payload isolated as inert text."
    );
}
