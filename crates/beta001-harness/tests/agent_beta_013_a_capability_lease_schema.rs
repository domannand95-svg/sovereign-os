...
impl AgentLeaseValidator {
    pub fn validate(value: &serde_json::Value) -> AgentLeaseValidationResult {
        ...
        if value.get("schema_version").and_then(|v| v.as_str()) != Some("REPOSITORY_AGENT_CAPABILITY_LEASE-v1") {
        ...
        if value.get("objective_reference").and_then(|v| v.as_str()).map_or(true, |s| s.is_empty()) {
            return AgentLeaseValidationResult::Invalid("Capability lease must be bound to a declared objective reference".into());
        }
        ...
        let allowed_keys = [
            "schema_version", "lease_id", "agent_identity", "capability_scope",
            "objective_reference", "temporal_bounds", "issuing_authority_reference"
        ];

        if let Some(obj) = value.as_object() {
            for key in obj.keys() {
                if !allowed_keys.contains(&key.as_str()) {
                    return AgentLeaseValidationResult::Invalid(format!("Authority escalation or credential injection detected: {}", key));
                }
            }
        }
        ...
        if let Some(scopes) = value.get("capability_scope").and_then(|v| v.as_array()) {
            if scopes.is_empty() {
                return AgentLeaseValidationResult::Invalid("Capability scope cannot be empty".into());
            }
            for scope in scopes {
                if let Some(s_str) = scope.as_str() {
                    if s_str == "all" || s_str.contains('*') {
                        return AgentLeaseValidationResult::Invalid("Wildcard capability scope forbidden".into());
                    }
                }
            }
        }
...
mod agent_capability_lease_tests {
    ...
    fn tc_agent_lease_001_valid_lease_accepted() {
        ...
        assert_eq!(AgentLeaseValidator::validate(&lease), AgentLeaseValidationResult::Valid);
    }
    ...
    fn tc_agent_lease_002_reject_expired_lease() {
        ...
        assert!(matches!(AgentLeaseValidator::validate(&lease), AgentLeaseValidationResult::Invalid(_)));
    }
    ...
    fn tc_agent_lease_004_reject_embedded_credentials() {
        let mut lease = get_valid_lease();
        lease.as_object_mut().unwrap().insert("token".to_string(), json!("secret_api_key_123"));
        assert!(matches!(AgentLeaseValidator::validate(&lease), AgentLeaseValidationResult::Invalid(_)));
    }
    ...
    fn tc_agent_lease_005_reject_authority_escalation_fields() {
        let mut lease = get_valid_lease();
        lease.as_object_mut().unwrap().insert("grant_admin".to_string(), json!(true));
        assert!(matches!(AgentLeaseValidator::validate(&lease), AgentLeaseValidationResult::Invalid(_)));
    }
    ...
    fn tc_agent_lease_007_reject_delegation_chain_expansion() {
        let mut lease = get_valid_lease();
        lease.as_object_mut().unwrap().insert("inherit_parent_capabilities".to_string(), json!(true));
        assert!(matches!(AgentLeaseValidator::validate(&lease), AgentLeaseValidationResult::Invalid(_)));
    }
}
