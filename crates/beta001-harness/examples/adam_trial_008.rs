...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "[1/6] Initializing ExternalApiBackend for [{}] at [{}]",
    ...
    println!(
        "[2/6] Captured raw transport bytes: size = {} bytes",
        response_size_bytes
    );
    ...
    println!(
        "[3/6] TransportPlane constructed (credential_present: {})",
        transport_plane.credential_present
    );
    assert!(
        !transport_plane.credential_present,
        "CRITICAL: Credential must never be present in evidence!"
    );
    ...
    if let Some(ref tp) = pkg.transport_plane {
        fs::write(
            artifact_dir.join("transport_plane.json"),
            serde_json::to_string_pretty(tp)?,
        )?;
    }
    fs::write(
        artifact_dir.join("candidate_plane.json"),
        serde_json::to_string_pretty(&pkg.candidate_plane)?,
    )?;
    fs::write(
        artifact_dir.join("containment_plane.json"),
        serde_json::to_string_pretty(&pkg.containment_plane)?,
    )?;
    fs::write(
        artifact_dir.join("integrity_plane.json"),
        serde_json::to_string_pretty(&pkg.integrity_plane)?,
    )?;
    fs::write(
        artifact_dir.join("evaluation_plane.json"),
        serde_json::to_string_pretty(&pkg.evaluation_plane)?,
    )?;
    fs::write(
        artifact_dir.join("package_digest.txt"),
        sealed_package.digest(),
    )?;

    println!(
        "Artifacts written successfully to: {}",
